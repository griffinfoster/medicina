\documentclass[useAMS,macros,usenatbib]{mn2e}
...
\begin{figure}
  \centering
  %\includegraphics[width=\linewidth]{graphics/layout.png}
  \includegraphics[scale=0.5]{graphics/layout.png}
...
For the the bright class A sources; Cas A, Cyg A, Tau A, and Virgo A, this time can be extended since they remain the dominating source well after crossing the FWHM.
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{{graphics/pb/best2_3_20_primary_beam}.pdf}
    ...
    The antennas have a sinc response to a point source.
...
\begin{figure*}
    ...
    \includegraphics[width=0.45\linewidth]{{graphics/fx/corr.2455996.22803.tau.s.ms.DATA.channel.1ch}.pdf}
    ...
    \includegraphics[width=0.45\linewidth]{{graphics/img.2455999.21403.tau.cal.s.ms.psf.channel.1ch.mod}.pdf}
    ...
    \includegraphics[width=0.45\linewidth]{{graphics/fx/corr.2455996.22803.tau.s.ms.CORRECTED_DATA.channel.1ch}.pdf}
    ...
    \includegraphics[width=0.45\linewidth]{{graphics/fx/corr.2455996.22803.tau.s.ms.CORRECTED_DATA.channel.1ch.restored}.pdf}
...
%weighting
\subsection{Weighting for a Regularly Gridded Array}

As noted, the regular gridded array produces many highly redundant baselines, Figure \ref{fig:fx_tau_psf}.
When forming the dirty image the choice of weighting greatly effects the outcome of the image.
In the highly redundant array case of BEST-2 the effects of using uniform versus natural weighting can be seen in Figure \ref{fig:fx_weighting}.
...
\begin{figure*}
    ...
    \includegraphics[width=0.45\linewidth]{{graphics/fx/corr.2455996.22803.tau.s.ms.natural.CORRECTED_DATA.channel.1ch}.pdf}
    \label{fig:fx_tau_natural}
    }
    \hspace{0.05\linewidth}
    \subfloat[Dirty image with uniform weighting of the point source Taurus A.]{
    \includegraphics[width=0.45\linewidth]{{graphics/fx/corr.2455996.22803.tau.s.ms.uniform.CORRECTED_DATA.channel.1ch}.pdf}
...
We use the FX correlator observations as a baseline to compare the results of observations from the spatial FFT imager.
...
Conversely, in the case of the spatial FFT correlator, gain corrections must be applied before the spatial FFT is performed, as the operation effectively averages together redundant baselines.
...
This effect that is likely to be temperature dependent, but may also depend on the declinationpointing angle.

\begin{figure*}
    \centering

    \subfloat[Gain phase variation over different observations]{
    \includegraphics[width=0.7\textwidth]{{graphics/gain_phs_var_deplane_crop}.pdf}
    \label{fig:gain_variation_phase}
    }

    \subfloat[Gain amplitude variation over different observations]{
    \includegraphics[width=0.9\textwidth]{{graphics/gain_amp_var_crop}.pdf}
...
\begin{figure*}
    ...
    \includegraphics[width=0.45\linewidth]{{graphics/img.2455985.25525.tau.uncal.s.ms.DATA.channel.1ch}.pdf}
    \label{fig:uncal_sfft}
    }
    \hspace{0.05\linewidth}
    \subfloat[Dirty image of Tau/3c144 with complex gain calibrations applied. This image has a signal to noise ratio around 100, a CLEAN method can be used to improve the image dynamic range.]{
    \includegraphics[width=0.45\linewidth]{{graphics/img.2455999.21403.tau.cal.s.ms.DATA.channel.1ch}.pdf}
    \label{fig:cal_sfft}
    }
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{{graphics/cas_transit_beams_6jun2011_crop}.pdf}
...
The spatial FFT imager outputs a set of 128 accumulated beam powers which can be converted to UV sampled baselines by performing an inverse FFT.
...
The flux scale is set by using the flux scale from FX data taken during the same observation.
...
The signal to noise of the images are comparable to that of the images formed with the FX correlator during the same observation, Figure \ref{fig:fx_tau}.
...
\begin{figure*}
    ...
    \includegraphics[width=0.45\linewidth]{{graphics/sfft/img.2455996.22807.tau.s.ms.DATA.channel.1ch}.pdf}
    ...
    \includegraphics[width=0.45\linewidth]{{graphics/sfft/img.2455996.22807.tau.s.ms.DATA.channel.1ch.restored}.pdf}
...
To make a comparison between images we define the dynamic range of each image to be the peak flux divided by the standard deviation of the noise in a empty region of the image.
...
For these snapshot images the spatial FFT produces essentially the same dynamic range as the FX correlator.
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{{graphics/bls/phase_plot_0_31_cyg}.png}
...
Looking at a highly redundant baseline, such as (0,4), we expect the individual FX baselines to have a higher noise figure than the spatial FFT effective baseline, Figure \ref{fig:bl_phase_0}.
...
This is shown to be correct, but the spatial FFT effective baseline has a higher noise standard deviation.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{{graphics/bls/phase_plot_0_4_tau}.png}
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{{graphics/bls/tau_bls_v2}.png}
