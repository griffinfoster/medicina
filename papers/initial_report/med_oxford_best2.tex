...
\begin{figure}
    ...
    \caption{Measured east-west primary beam based on the correlation between antennas 1N-6-1 and 1N-1-3, which are representitive of typical antennas, based on a Cassiopeia A transit. The dashed lines indicate the FWHM points. The antennas should have a sinc response to a point source, as expected the first sidelobes are -15 dB down from the peak.}
    \label{fig:best2_pb}
\end{figure}
...
Phase calibration and setting the flux scale is accomplished using Casseiopia A and Cygnus A observations as point source sky models set to their 3C flux levels and known spectral indices.
...
After cleaning the we measure the residual noise to be around 2 Jy over a number of two minute snapshot observations.
...
\begin{figure}
    ...
    \subfloat[Observation of the Cassiopeia A field on Julian Date 2455985, the flux has been fixed to 5600 Jy with a dynamic range of $\sim900$. The structure on the edge of the image are likely from the sun begining to come into the field of view.]{
    ...
    \hspace{10pt}
    \subfloat[Observation of the same field ten days later on Julian Date 2455996. The Sun is slightly resolved in the North-South direction. The sun is at declination $-4^{\circ}$, about $62^{\circ}$ from the field of view but is within one of the primary beam sidelobes and has an apparent declination position at $58^{\circ}$. Using the same flux scale from the previous Cassiopeia A observation the Sun is measured to have a flux of 6000 Jy. Though, since the sun is being observed through a sidelobe this value is attenuated compared to the actual value.]{
...
\subsection{Spatial FFT Results and Comparison to the FX Correlator}
\label{sfft_results}
...
\begin{figure}
    ...
    \hspace{10pt}
    \subfloat[Cleaned image of the Taurus field. The cleaning process brings the dyanmic range up to around 300, similar to the value from the FX correlator image in fig. \ref{fig:fx_tau_clean}.
...
The dynamic range of the spatial FFT appears to be approximately the same as the FX correlator but there is a low level distortion in the spatial FFT images in a few of the sources.
To understand this we can look at the baseline data to see how that varies between the two measurements.
...
\begin{figure}
    ...
    \caption{Phase noise for each baseline during observation plotted against the the number of redundant baselines.
    Red dots are for the FX correlator.
    Black circles are the phase noise after averaging together a set of redundant baselines.
    This expectedly decreases as $\frac{1}{\sqrt{n}}$ on the more redundant spacings.
    Green squares are the spatial FFT baselines, which are effectively an average of all the redundant spacing baselines.
...
%what has been accomplished: sfft has proven to work at a similar level of quality to the fx correlator but with additional noise
%further work on SFFT calibration: understanding increased phase noise, image quality
