...
The integration time is sufficently short that this does not effect the phase.
...
The UV coverage is essentialy the same for any observation, with some variation due to pointing declination. 
...
\begin{figure}
    ...
    \caption{A measured east-west primary beam based on the correlation between antennas 1N-6-1 and 1N-1-3, which are representitive of typical antennas, based on a Cassiopeia A transit. The dashed lines indicate the FWHM points. The antennas should have a sinc response to a point source, as expected the first sidelobes are -15 dB down from the peak.}
    \label{fig:best2_pb}
\end{figure}
...
An effect of the density of the antenna layout is a low point spread function(PSF) to field of view ratio (spatial fidelity), thus images tend to contain at most a few spatial seperate point sources.
...
Also, in that time the Sun was undergoing a very active period which maybe one of the reasons we observed it at with a particularlly strong flux.
...
Figures \ref{fig:fx_cas_nosun} and \ref{fig:fx_cas_sun} show the Cassiopeia A field seperated by 10 days, in that time the sun has moved well into the center of the field.
...
As noted, the regular gridded array produces many highly redundant baselinesi, fig. \ref{fig:fx_tau_psf}.
...
\section{Spatial FFT Results and Comparison to the FX Correlator}
\label{sfft_results}
...
%use peak transit of A class sources to derive gains based on a point source model
