...
def make_antenna(config='config.xml', num_rows=1):
    ...
    * POLCALB:  As above, for feed B
    ...
    c.append(pf.Column(name='NOPOL', format='1J',\
        array=np.zeros(num_rows,dtype=np.int)))
...
def make_frequency(config='config.xml', num_rows=1):
    ...
    * BB_CHAN:  ? 
    ...
    c.append(pf.Column(name='BB_CHAN',  format='4J',\
        array=np.zeros(num_rows,dtype='int32')))
...
def make_uv_data(config='config.xml', num_rows=1, n_chans=1024):
    ...
    * FLUX         UV visibility data matrix
    ...
    c.append(pf.Column(name='WEIGHT', format=fmt,\
        array=np.ones(num_rows,dtype=dtype)))
      
    c.append(pf.Column(name='FLUX', format=fmt,\
