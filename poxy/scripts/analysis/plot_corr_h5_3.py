...
for fi, fname in enumerate(fnames):
    ...
    if fi==0:
        ...
        d = numpy.zeros([len(time_index)*len(fnames),len(chan_index),len(bl_index),len(pols),2], dtype=fh['xeng_raw0'].dtype)
        file_len = len(time_index)
        if len(time_index) > len(chan_index):
            for t in time_index:
                d[t] = fh.get('xeng_raw0')[t,chan_index][:,bl_index][:,:,pols]
        else: 
        ...
        if len(time_index) > len(chan_index):
            for t in time_index:
                d[file_len*fi+t] = fh.get('xeng_raw0')[t,chan_index][:,bl_index][:,:,pols]
        else:
