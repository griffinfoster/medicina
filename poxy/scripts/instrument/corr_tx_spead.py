...
class CasperN_Packet:
    ...
    def pack_from_prms(self, timestamp, xeng, offset, heap_len, data):
        ...
        option5 = (1<<63) + ((0x1700+xeng)<<40) + timestamp
        option6 = (0<<63) + ((0x1900+xeng)<<40) + 0
...
class CorrTX:
    def __init__(self, pid, endian='>',ip='10.0.0.1', x_per_fpga=2, port=7147, payload_len=4096, verbose=False, timestamp_rnd=1024*1024):
        ...
        for x in range(x_per_fpga):
            ...
            xeng_file=(open('/proc/%i/hw/ioreg/inst_seng_id%i'%(pid,x),'r'))
            xeng_file.seek(2)
            self.xeng.append(struct.unpack('>H',xeng_file.read(2))[0])
            xeng_file.close()
	    #self.xeng.append(x) #Alternative to inst_seng_id reg, for only one roach
    ...
    def _tx(self):
        ...
        while True:
            ...
            for xnum in range(self.x_per_fpga):
                print '[%6i] Grabbed %i vectors for X engine %i with timestamp %i (increment of %i) (diff %4.2fs).'%(n_integrations,int_xeng_vectors[xnum], self.xeng[xnum], timestamp[xnum], timestamp[xnum]-last_timestamp[xnum],realtime_diff[xnum])
		last_timestamp[xnum] = timestamp[xnum]
