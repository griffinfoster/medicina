#!/usr/bin/env python
'''
Grabs the contents of "snap_vacc0" (one per FPGA) at the output of the X eng
...
if __name__ == '__main__':
    ...
    p = OptionParser()
    p.set_usage('xeng_snap_vacc.py [options] CONFIG_FILE')
    p.add_option('-m', '--manual', dest='manual', action='store_true', default=False, help='Manually trigger snap block.')
...
try:
    ...
    sys.stdout.flush()
    bram_dmp=xeng.xeng_snap(dev_name, brams, man_trig = manual, wait_period = 2)
    print 'done.'
    ...
    for x, fpga in enumerate(xeng.xfpgas):
        ...
        for li in range(len(bram_data[x]) / 2):
            ...
            freq = (index / n_stokes / n_bls) * x_per_fpga * len(xeng.xfpgas) + xeng
            ...
            if (real_val != 0) or (imag_val != 0) or opts.verbose:
                #print '[%s] [%4i]: Freq: %i. Stokes: %i. Raw value: 0x%05x + 0x%05xj (%6i + %6ij).'%(xeng.servers[xeng][0], index, freq, stokes, real_val, imag_val, real_val, imag_val)
                print '[%s] [%4i]: Freq: %i. Stokes: %i. Raw value: 0x%05x + 0x%05xj (%i + %ij).'%(xeng.servers[x][0], index, freq, stokes, real_val, imag_val, real_val, imag_val)
