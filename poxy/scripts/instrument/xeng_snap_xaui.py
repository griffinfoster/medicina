...
'''
import time, sys, numpy, os, katcp, socket, struct, stats
...
if __name__ == '__main__':
    ...
    p = OptionParser()
    p.set_usage('xeng_snap_xaui.py [options] CONFIG_FILE')
...
try:
    ...
    bram_dmp=xeng.xeng_snap(dev_name,brams,man_trig=man_trigger,wait_period=3,offset=opts.offset*num_bits*2*2/64*packet_len)

    print 'Unpacking bram out of band contents...',
    sys.stdout.flush()
    bram_oob=dict()
    for f in range((xeng.n_ants/xeng.n_ants_per_feng/xeng.n_xaui)):
        if len(bram_dmp[brams[2]][f])<=4:
            print '\n   No data for X engine %s.'%xeng.servers[f]
            bram_oob[f]={}
        else:
            if opts.verbose:
                print '\n   Got %i values from %s.'%(len(bram_dmp['bram_oob'][f])/4,xeng.servers[f])
    ...
    for f in range((xeng.n_ants/xeng.n_ants_per_feng/xeng.n_xaui)):
        ...
        for i in range(0,len(bram_dmp[brams[2]][f])/4):
            if opts.verbose:
                ...
                print '[%s @ %4i]: %016X'%(xeng.servers[f][0],i,pkt_64bit),
            ...
            if bram_oob[f]['linkdn'][i]:
                print '[%s] LINK DOWN AT %i'%(xeng.servers[f][0],i)

            #elif bram_oob[f]['adc'][i]:
            #    adc,amp = struct.unpack('>II',bram_dmp['bram_msb'][f][(4*i):(4*i)+4]+bram_dmp['bram_lsb'][f][(4*i):(4*i)+4])
            #    adc_ant,adc_pol=im.get_ant_location(f,target_xaui,adc)
            #    print '[%s][%i] ADC amplitude update for input %i (ant%i%s) with raw value 0x%08x. (average ADC count %3f)'%(xeng.servers[f],i,adc,adc_ant,adc_pol,amp,numpy.sqrt(float(amp)/adc_levels_acc_len))
    ...
    #            print ('HEADER RECEIVED')
                ...
                print '[%s] [Pkt@ %4i Len: %2i]     (MCNT %16u ANT: %1i, Freq: %4i)  RMS: X: %1.2f Y: %1.2f.  {X: %1.2f+%1.2fj (%2.1f & %2.1f bits), Y:%1.2f+%1.2fj (%2.1f & %2.1f bits)} {Pk: X,Y: %1.2f,%1.2f (%2.1f,%2.1f bits)}'%(xeng.servers[f][0],\
                    pkt_hdr_idx,\
